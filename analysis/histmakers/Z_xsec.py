...
def build_graph_qq(df, dataset):
    ...
    ### FILTERS TO BE USED
    #########
    # Comment out the next line if you want to plot Evis/sqrt(s)
    #df = df.Filter("sum_energy>45.559")
    results.append(df.Histo1D(("cutFlow", "", *bins_cut), "cut1"))
    # Comment out the next line if you want to plot Longitudinal Energy Imbalance
    df = df.Filter("energy_imbalance_long < 0.6")
    results.append(df.Histo1D(("cutFlow", "", *bins_cut), "cut2"))
    # Comment out the next line if you want to plot Transverse Energy Imbalance
    df = df.Filter("energy_imbalance_trans <0.6")
    results.append(df.Histo1D(("cutFlow", "", *bins_cut), "cut3"))
    # Comment out the next line if you want to plot Number of Particles
    df = df.Filter("(abs(cos_thetat) <= 0.74 && RP_no > 13) || (abs(cos_thetat) > 0.74 && RP_no > 17)")
    results.append(df.Histo1D(("cutFlow", "", *bins_cut), "cut4"))
    # Leave the next line commented out for all plots except Nparticles. Change the filter between "abs(cos_thetat) <= 0.74" or "abs(cos_thetat) > 0.74" depending on which you want 
    #df = df.Filter("abs(cos_thetat) > 0.74")
    ...
    # results.append(df.Histo1D(("et", "", *bins_et), "et"))
    # results.append(df.Histo1D(("ep", "", *bins_et), "ep"))
...
if __name__ == "__main__":
    ...
    if args.flavor == "qq":
        datasets += functions.filter_datasets(datasets_spring2021_ecm91, ["wzp6_ee_qq_ecm91p2", "wzp6_ee_tautau_ecm91p2", "wzp6_ee_mumu_ecm91p2", "wzp6_gaga_qq_5_ecm91p2", "p8_ee_Zee_ecm91", "kkmc_ee_uu_ecm91p2", "kkmc_ee_dd_ecm91p2", "kkmc_ee_cc_ecm91p2", "kkmc_ee_ss_ecm91p2", "kkmc_ee_bb_ecm91p2", "kkmc_ee_uu_ecm91p2_noISR"]) #,"wzp6_ee_qq_ecm89p5","wzp6_ee_tautau_ecm89p5","wzp6_ee_mumu_ecm89p5","wzp6_ee_qq_ecm90p2","wzp6_ee_tautau_ecm90p2","wzp6_ee_mumu_ecm90p2","wzp6_ee_qq_ecm92p0","wzp6_ee_tautau_ecm92p0","wzp6_ee_mumu_ecm92p0","wzp6_ee_qq_ecm93p0","wzp6_ee_tautau_ecm93p0","wzp6_ee_mumu_ecm93p0"]) #"p8_ee_Zuds_ecm91", "p8_ee_Zcc_ecm91", "p8_ee_Zbb_ecm91", "wzp6_ee_qq_ecm91p2"])
        if args.jetAlgo == "kt":
            result = functions.build_and_run(datasets, build_graph_qq, "tmp/output_z_xsec_qq_kt.root", maxFiles=args.maxFiles, norm=True, lumi=44.84)
        if args.jetAlgo == "valencia":
            result = functions.build_and_run(datasets, build_graph_qq, "tmp/output_z_xsec_qq_valencia.root", maxFiles=args.maxFiles, norm=True, lumi=44.84)
        if args.jetAlgo == "genkt":
            result = functions.build_and_run(datasets, build_graph_qq, "tmp/output_z_xsec_qq_genkt.root", maxFiles=args.maxFiles, norm=True, lumi=44.84)
