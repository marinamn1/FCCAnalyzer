...
parser.add_argument("--limitingTheta", type=float, help="Limiting Theta angle (size of the hole, radians)", default=0.001)
...
def build_graph_qq(df, dataset):
    ...
    df = df.Define("RP_charged", "FCCAnalyses::charged(ReconstructedParticles)")
    ...
    #df = df.Define("sum_energy_vis", "FCCAnalyses::sum_energy(RP_e_vis)")
    #df = df.Define("sum_energy_NonVisible", "FCCAnalyses::sum_energy(RP_e_NonVisible)")
    ...
    if args.jetAlgo == "kt":
        df = df.Define("clustered_jets", "JetClustering::clustering_ee_kt(0, 10, 0, 10)(pseudo_jets)")
        ...
        df = df.Define("clustered_jets", "JetClustering::clustering_ee_genkt(0.5, 0, 5, 0, 0, -1)(pseudo_jets)")
    ...
    results.append(df1.Histo1D(("discard1RP_e_vis", "", *jet_energy), "RP_e_vis"))
    results.append(df2.Histo1D(("discard2RP_e_vis", "", *jet_energy), "RP_e_vis"))
    ...
    results.append(df.Histo1D(("electrons_theta", "", *bins_phi), "electrons_theta"))
    results.append(df.Histo1D(("muons_no", "", *bins_count), "muons_no"))
    results.append(df.Histo1D(("muons_theta", "", *bins_phi), "muons_theta"))
    results.append(df.Histo1D(("RP_theta", "", *bins_phi), "RP_theta"))
    results.append(df1.Histo1D(("discard1RP_theta", "", *bins_phi), "RP_theta_vis"))
    results.append(df2.Histo1D(("discard2RP_theta", "", *bins_phi), "RP_theta_vis"))
    results.append(df.Histo1D(("RP_theta_NonVisible", "", *bins_phi), "RP_theta_NonVisible"))
    ...
    results.append(df1.Histo1D(("discard1sum_energy", "", *dijet_m), "sum_energy"))
    results.append(df2.Histo1D(("discard2sum_energy", "", *dijet_m), "sum_energy"))
    #results.append(df.Histo1D(("sum_energy_vis", "", *dijet_m), "sum_energy_vis"))
    #results.append(df.Histo1D(("sum_energy_NonVisible", "", *dijet_m), "sum_energy_NonVisible"))
...
if __name__ == "__main__":
    ...
    if args.flavor == "qq":
        datasets += functions.filter_datasets(datasets_spring2021_ecm91, ["wzp6_ee_qq_ecm91p2", "wzp6_ee_tautau_ecm91p2", "wzp6_ee_mumu_ecm91p2", "wzp6_gaga_qq_5_ecm91p2","kkmc_ee_uu_ecm91p2", "kkmc_ee_dd_ecm91p2", "kkmc_ee_cc_ecm91p2", "kkmc_ee_ss_ecm91p2", "kkmc_ee_bb_ecm91p2", "p8_ee_Zee_ecm91", "p8_ee_Zmumu_ecm91", "p8_ee_Ztautau_ecm91"]) 
        if args.jetAlgo == "kt":
        ...
        if args.jetAlgo == "genkt":
            result = functions.build_and_run(datasets, build_graph_qq, "tmp/output_hadron_xsec_genkt.root", maxFiles=args.maxFiles, norm=True, lumi=75000000)
