...
parser.add_argument("--maxFiles", type=int, help="Max number of files (per dataset)", default=11)
...
def build_graph_qq(df, dataset):
    ...
    df = df.Define("leps_all_no", "FCCAnalyses::ReconstructedParticle::get_n(ReconstructedParticles)")
...
if __name__ == "__main__":
    ...
    if args.flavor == "mumu":
        ...
        result = functions.build_and_run(datasets, build_graph_ll, "tmp/output_z_xsec_mumu_genkt.root", maxFiles=args.maxFiles) # , norm=True, lumi=150000000
    if args.flavor == "ee":
        datasets += functions.filter_datasets(datasets_spring2021_ecm91, ["p8_ee_Zee_ecm91", "p8_ee_Ztautau_ecm91"])
        result = functions.build_and_run(datasets, build_graph_ll, "tmp/output_z_xsec_ee_genkt.root", maxFiles=args.maxFiles, norm=True, lumi=150000000)
    if args.flavor == "qq":
