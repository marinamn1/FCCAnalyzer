...
Vec_rp VisibleParticles(float limit, Vec_rp in){
...
Vec_rp neutral(Vec_rp in){
    Vec_f charges = FCCAnalyses::ReconstructedParticle::get_charge(in);
    Vec_rp result;
    for(int i=1;i<in.size();i++){
        if (charges[i]==0){
        result.push_back(in[i]);
